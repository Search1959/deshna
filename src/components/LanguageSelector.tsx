  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        onClick={() => setIsOpen(!isOpen)}
        className="flex items-center space-x-1.5 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl border-2 border-amber-200 bg-amber-50 hover:bg-amber-100/90 text-amber-950 text-xs font-black shadow-xs transition cursor-pointer min-h-[38px]"
        title="Change Indian Language (বাংলা, हिन्दी, etc.)"
      >
        <Globe className="w-4 h-4 text-amber-600 shrink-0" />
        <span className="font-extrabold text-amber-950 tracking-tight">
          {currentLang.nativeName.split(' ')[0]}
        </span>
        <ChevronDown className={`w-3.5 h-3.5 text-amber-700 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
      </button>
