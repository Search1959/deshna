...
export const MobileMenuDrawer: React.FC<MobileMenuDrawerProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden flex justify-end">
      {/* Dark backdrop overlay covering full screen */}
      <div
      ...
      {/* Main Drawer Panel: Full height, sleek right docking, responsive width */}
      <aside
        aria-label="Mobile Navigation Menu"
        className="relative w-full max-w-[340px] sm:max-w-sm h-full bg-white border-l-4 border-amber-400 shadow-2xl flex flex-col justify-between z-10 animate-in slide-in-from-right duration-200"
      >
        {/* Top Header */}
        <div className="p-4 border-b-2 border-amber-200/80 bg-gradient-to-r from-amber-100/90 to-amber-50 flex items-center justify-between shrink-0">
          <button
          ...
          >
            ...
            <div>
              <span className="font-black text-slate-900 text-base flex items-center gap-1.5">
                DESHNA <span className="text-[10px] bg-amber-200 text-amber-900 px-1.5 py-0.5 rounded font-black border border-amber-300">AI HUB</span>
              </span>
              <p className="text-[11px] font-bold text-amber-800">{t('tagline', 'Learn Smarter • Practice Better')}</p>
            </div>
          </button>
          <button
            onClick={onClose}
            className="p-2 rounded-xl text-slate-700 hover:bg-amber-200/80 transition"
        ...
        <div className="p-4 space-y-4 overflow-y-auto flex-1 divide-y divide-slate-100">
          ...
          <div className="space-y-2 pb-1">
            <button
              ...
              className="w-full flex items-center justify-between px-3.5 py-2 bg-amber-50/90 hover:bg-amber-100 border-2 border-amber-200 rounded-xl text-xs font-bold text-amber-950 transition"
            >
              <div className="flex items-center space-x-2">
                <Search className="w-4 h-4 text-amber-600" />
                <span>{t('search_placeholder', 'Search subjects, chapters, Qs...')}</span>
              </div>
            ...
            <button
              ...
              className="w-full py-2 px-3 bg-amber-400 hover:bg-amber-500 text-slate-950 rounded-xl font-black text-xs flex items-center justify-center space-x-2 shadow-xs transition"
            >
          ...
          <div className="pt-3 space-y-1">
            <div className="text-[11px] font-black uppercase tracking-wider text-slate-400 px-2 py-0.5">
            ...
            <div className="grid grid-cols-1 gap-1">
              ...
              >
                <div className="flex items-center space-x-3">
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center bg-amber-100 text-amber-800">
                    <Home className="w-4 h-4" />
                  </div>
                  <span>Home (मुख्य पृष्ठ / হোম)</span>
                </div>
              ...
              >
                <div className="flex items-center space-x-3">
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center bg-blue-100 text-blue-800">
                    <BookOpen className="w-4 h-4" />
                  </div>
                  <div>
                    <span className="font-bold">{t('classes', 'Classes & Curriculum')}</span>
                    <span className="block text-[10px] text-slate-500 font-medium">Grades 1–11 (CBSE, ICSE, State)</span>
                  </div>
                </div>
                <ChevronRight className="w-3.5 h-3.5 text-slate-400" />
              </button>
              ...
              >
                <div className="flex items-center space-x-3">
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center bg-emerald-100 text-emerald-800">
                    <GraduationCap className="w-4 h-4" />
                  </div>
                  <div>
                    <span className="font-bold">{t('study_mode', 'Study Dashboard')}</span>
                    <span className="block text-[10px] text-slate-500 font-medium">Adaptive Practice & Syllabus</span>
                  </div>
                </div>
                <ChevronRight className="w-3.5 h-3.5 text-slate-400" />
              </button>
          ...
          <div className="pt-3 space-y-1">
            ...
            <div className="grid grid-cols-1 gap-1">
              {[
                ...
                {
                  id: 'exam_prep',
                  name: t('exam_prep', 'Board & Exam Prep'),
                  sub: 'Adaptive mock tests & timer',
                  icon: Zap,
                  color: 'text-amber-700 bg-amber-100',
                  action: () => navigateTo('exam_prep'),
                },
              ].map((item) => {
            ...
            <div className="p-3 bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-2xl space-y-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2.5">
                  ...
                  />
                  <div>
                    <div className="font-black text-xs text-slate-900">{currentStudent.name}</div>
                ...
                <button
                  ...
                  className="text-[11px] font-black px-2 py-1 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition flex items-center space-x-1"
                >
                  <span>Switch</span>
                  ...
                  {allStudents.map((s) => (
                    ...
                    >
                      <div className="flex items-center space-x-2">
                        ...
                        <span>{s.name} (Grade {s.gradeId})</span>
                      </div>
                      {s.id === currentStudent.id && <span className="text-[10px] text-blue-700 font-black">Active</span>}
                    </button>
                  ))}
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-1.5 pt-1">
                <div className="flex items-center space-x-1 text-[11px] font-bold text-amber-800 bg-amber-100/80 px-2 py-0.5 rounded-md">
                  <Flame className="w-3 h-3 text-amber-500 fill-amber-500" />
                  <span>{currentStudent.streakDays}d Streak</span>
                </div>
                <div className="flex items-center space-x-1 text-[11px] font-bold text-emerald-800 bg-emerald-100/80 px-2 py-0.5 rounded-md">
                  <Star className="w-3 h-3 text-emerald-500 fill-emerald-500" />
                  <span>{currentStudent.totalPoints} XP</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="p-3 bg-slate-50 border-t border-slate-200 flex items-center justify-between gap-2 shrink-0">
          <button
            ...
            className={`flex-1 flex items-center justify-center space-x-1.5 py-2.5 rounded-xl text-xs font-black border transition ${
              isSpeaking
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center space-x-1.5 py-2.5 bg-gradient-to-r from-pink-500 to-rose-500 text-white rounded-xl text-xs font-black shadow-sm"
          >
